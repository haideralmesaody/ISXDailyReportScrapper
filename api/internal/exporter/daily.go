...
func (d *DailyExporter) ExportDailyReports(records []domain.TradeRecord, outputDir string) error {
	...
	for _, dateKey := range dates {
		...
		// Check if any actual trading occurred on this date
		// Skip dates with only forward-filled data (weekends, holidays, non-trading days)
		hasActualTrading := false
		for _, record := range dayRecords {
			if record.TradingStatus {  // true = actively traded, false = forward-filled
				hasActualTrading = true
				break
			}
		}

		// Skip dates with no actual trading
		if !hasActualTrading {
			continue
		}
