...
[Setup]
; Unique identifier for this application (different from x64)
AppId={{C8D15E3C-8A5E-4F47-9F2D-123456789ABC}
AppName={#MyAppNameLong} (ARM64)
...
OutputDir=..\
OutputBaseFilename=ISX-Daily-Reports-Professional-ARM64-Installer
...
; Architecture and privileges - ARM64 specific
...
VersionInfoProductName={#MyAppNameLong} (ARM64)
...
WelcomeLabel2=This will install [name/ver] on your computer.%n%nThis professional installer will download the latest ARM64 release from GitHub and set up ISX Daily Reports with desktop shortcuts and Start Menu entries.%n%nThis version is optimized for ARM64 Windows devices like Surface Pro X and ARM laptops.%n%nIt is recommended that you close all other applications before continuing.
...
Name: "{group}\{#MyAppName} (ARM64)"; Filename: "{app}\{#MyAppExeName}"; Comment: "{#MyAppDescription}"; IconFilename: "{app}\app-icon.svg"
...
Name: "{group}\Uninstall {#MyAppName} (ARM64)"; Filename: "{uninstallexe}"; Comment: "Uninstall {#MyAppName}"

; Desktop shortcut (optional)
Name: "{autodesktop}\{#MyAppName} (ARM64)"; Filename: "{app}\{#MyAppExeName}"; Comment: "{#MyAppDescription}"; Tasks: desktopicon; IconFilename: "{app}\app-icon.svg"

; Quick Launch shortcut (optional, Windows XP/Vista/7)
Name: "{userappdata}\Microsoft\Internet Explorer\Quick Launch\{#MyAppName} (ARM64)"; Filename: "{app}\{#MyAppExeName}"; Tasks: quicklaunchicon; IconFilename: "{app}\app-icon.svg"

[Run]
; Download and install the application with ARM64 preference
Filename: "powershell.exe"; Parameters: "-ExecutionPolicy Bypass -File ""{tmp}\download-github-release.ps1"" -InstallPath ""{app}"" -AppName ""{#MyAppName}"" -Architecture ""ARM64"""; StatusMsg: "Downloading and installing {#MyAppName} for ARM64..."; Flags: waituntilterminated
...
begin
  ...
  if not IsARM64 then
  begin
    MsgBox('This installer is specifically designed for ARM64 Windows systems.'#13#10#13#10'For x64 systems, please use the x64 installer instead.', mbError, MB_OK);
    Result := False;
    Exit;
  end;
  ...
  begin
    V := MsgBox(ExpandConstant('An existing installation of {#MyAppName} (ARM64) was detected. Do you want to uninstall it first?'), mbInformation, MB_YESNO);
    if V = IDYES then
    begin
