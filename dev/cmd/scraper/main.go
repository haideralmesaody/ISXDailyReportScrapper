...
func checkLicense(stateFilePath string) bool {
	...
	if stateFilePath != "" {
		fmt.Printf("🔍 Checking license state file: %s\n", stateFilePath)
		valid, err := licenseManager.ValidateStateFile(stateFilePath)
		if err != nil {
			fmt.Printf("⚠️  State file validation error: %v\n", err)
			// Continue with normal validation
		} else if valid {
			fmt.Println("✅ License validated via state file")
			fmt.Println("═══════════════════════════════════════════════")
			return true
		} else {
			fmt.Println("⚠️  State file invalid or expired, proceeding with normal validation")
		}
	}
